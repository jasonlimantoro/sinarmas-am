...
function addCommas(a){a+="",x=a.split("."),x1=x[0],x2=x.length>1?"."+x[1]:"";for(var t=/(\d+)(\d{3})/;t.test(x1);)x1=x1.replace(t,"$1,$2");return x1+x2}var th_arr=["Danamas Rupiah Plus","Danamas Stabil","Simas Danamas Mantap Plus","Danamas Dollar","Simas Danamas Instrumen Negara","Danamas Pasti","Simas Syariah Berkembang","Simas Satu","Danamas Fleksi","Simas Satu Prima","Simas Syariah Unggulan","Simas Danamas Saham","Simas Saham Unggulan","Simas Saham Bertumbuh","Simas Saham Maksima"],prod_daily_nav_url="http://trading.simasnet.com/ROL/prod_daily_nav.php",prod_suku_bunga_url="http://trading.simasnet.com/ROL/prod_suku_bunga.php",request1=$.getJSON(prod_daily_nav_url,function(a){var t=a.results[0].nab_dt;$(".updated-date").append(t),$.each(a.results,function(t,s){$.each(th_arr,function(s,n){if(n===a.results[t].nama){var r=addCommas(a.results[t].nab);$("#nab"+s).html(function(){return"<td>"+r+"</td>"});var u=addCommas(a.results[t].ytd);$("#nab"+s).siblings().html(function(){return"<td>"+u+"%</td>"});var e=a.results[t].rating,i=5-e,d="★ ".repeat(e),m="_  ".repeat(i);$("th:contains('"+n+"')").siblings("th").html(function(){return"<div class='rating'>"+d+m+"</div>"})}})})}),request2=$.getJSON(prod_suku_bunga_url,function(a){var t=a.results[0].tgl_update;$(".update-date-sb").append(t),$.each(a.results,function(t,s){var n=a.results[t].tipe,r=a.results[t].suku_bunga;$(".body_suku_bunga").append("<tr><td>"+n+"</td><td>"+r+" %</td></tr>")})});
var offset=900,duration=500;$(window).scroll(function(){$(this).scrollTop()>offset?$(".back-to-top").fadeIn(duration):$(".back-to-top").fadeOut(duration)}),$(".back-to-top").click(function(o){return o.preventDefault(),$("html, body").animate({scrollTop:0},duration),!1});
function calculate_try_periodic(){var result_ro_array=["13,335,054.81","12,000,000,00","1,335,054.81","11,13%"];for(i=0;i<result_ro_array.length;i++)$("#result_ro"+eval("i+3")).html("<td><code>"+result_ro_array[i]+"</code></td>"),$("#code"+eval("i+3")).html("<code>"+result_ro_array[i]+"</code>")}function getProductCode(t){switch($(t).val()){case"Simas Satu":product_code="002";break;case"Danamas Pasti":product_code="005";break;case"Danamas Rupiah Plus":product_code="014";break;case"Simas Danamas Saham":product_code="020";break;case"Simas Satu Prima":product_code="049";break;case"Simas Saham Unggulan":product_code="050";break;default:product_code="005"}return $("#product_code").attr("value",product_code),product_code}function valid_date(t,e){return e-t>0}function stringToDate(t,e,a){var d=e.toLowerCase().split(a),r=t.split(a),n=d.indexOf("mm"),_=d.indexOf("dd"),o=d.indexOf("yyyy"),i=parseInt(r[n]);return i-=1,new Date(r[o],i,r[_])}function datetoString(t){return month=t.getMonth()+1,date=t.getDate(),year=t.getFullYear(),t.getMonth()+1<10&&(month="0"+month),t.getDate()<10&&(date="0"+date),date_complete_str=month+"/"+date+"/"+year,date_complete_str}function diffMonth(t,e){var a=t.getFullYear(),d=e.getFullYear(),r=t.getMonth();return 12*(d-a)+(e.getMonth()-r)+1}function sum(t,e){return t+e}function calculate_periodic(){product_type=$("#field_product").val(),invest_amount=$("#field_invest").val(),period_date=$("#field_periode_invest").val(),start_date=period_date,start_month_year=$("#field_bulan_tahun_awal").val(),""!=start_date&&""!=start_month_year&&(start_date_temp=start_month_year+"-"+start_date,start_date_obj=stringToDate(start_date_temp,"yyyy-mm-dd","-"),start_date_str=datetoString(start_date_obj)),end_date=$("#field_tanggal_akhir").val(),""!=end_date&&(end_date_temp=end_date,end_date_obj=stringToDate(end_date_temp,"yyyy-mm-dd","-"),end_date_str=datetoString(end_date_obj)),null==product_type||""==invest_amount||null==period_date||""==start_month_year||""==end_date?alert("Masukkan Data yang lengkap!"):valid_date(start_date_obj,end_date_obj)?(month_diff=diffMonth(start_date_obj,end_date_obj),nab_url="http://trading.simasnet.com/ROL/prod_chart_daily.php",$.post(nab_url,{product_code:getProductCode("#field_product"),start_date:start_date_str,end_date:end_date_str},function(t){var e=t.results;if(start_nab="unvailable",end_nab="unvailable",0==e.length)alert("Tidak ada data untuk range tanggal tersebut!");else if(e[0].date==start_date_str?start_nab=e[0].nab:alert("NAB Awal tidak ada data!"),e[e.length-1].date==end_date_str?end_nab=e[e.length-1].nab:alert("NAB akhir tidak ada data"),"unvailable"!=start_nab&&"unvailable"!=end_nab){var a=[],d=[start_date_str],r=[];$.each(e,function(t){a.push(e[t].date)});for(_=0;_<month_diff-1;_++)date_increment=new Date(start_date_obj.setMonth(start_date_obj.getMonth()+1)),date_increment_str=datetoString(date_increment),d.push(date_increment_str);for(_=0;_<d.length;_++)if(find_array_index=a.indexOf(d[_]),-1!=find_array_index)r.push(e[find_array_index].nab);else{date_obj_escape=stringToDate(d[_],"mm/dd/yyyy","/");for(var n=0;n<e.length;n++)if(date_increment_obj_escape=new Date(date_obj_escape.setDate(date_obj_escape.getDate()+1)),date_increment_str_escape=datetoString(date_increment_obj_escape),find_array_index_escape=a.indexOf(date_increment_str_escape),-1!=find_array_index_escape){r.push(e[find_array_index_escape].nab);break}}up_list=[];for(_=0;_<r.length;_++)up_data=invest_amount/r[_],up_list.push(up_data);console.log(e),console.log(r),console.log(d),unit=up_list.reduce(sum).toFixed(6),total_value=(unit*end_nab).toFixed(2),principal_value=(invest_amount*month_diff).toFixed(2),profit_loss=(total_value-principal_value).toFixed(2),profit_loss_p=(profit_loss/principal_value*100).toFixed(3),ans=[start_nab,end_nab,unit,total_value,principal_value,profit_loss,profit_loss_p],profit_loss<0&&($("#result5").css("color","red"),$("#result6").css("color","red"));for(var _=0;_<ans.length;_++)document.getElementById("result"+_).innerHTML=addCommas(ans[_])}})):alert("Tanggal awal lebih besar daripada tanggal akhir!")}$("#field_periode_invest").change(function(){var t=$("#field_periode_invest").val();$("#field_tanggal_awal").attr("value",t)});
jQuery(document).ready(function(e){e("#menu").mmenu({extensions:["fx-menu-zoom"],offCanvas:{pageSelector:"#page-wrapper"},navbars:[{position:"top",content:["searchfield"]},{position:"top"},{position:"bottom",content:["<a class='fa fa-twitter' href='#/'></a>","<a class='fa fa-facebook' href='#/'></a>"]}]})});
var custom_panel_class="panel-primary";$(".real-time .panel").addClass(custom_panel_class);